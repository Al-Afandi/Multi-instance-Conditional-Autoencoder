...
for k in range(repeat):
    ...
    for epoch in range(epoch_index,num_epochs):
        ...
        if epoch%4==0:
            ...
            sf.write('./logs/audio/records/celebrity_MIAE_ground_truth_audio'+str(k)+'_'+str(epoch)+'.wav', ground_truth[0,0], fs)
            sf.write('./logs/audio/records/celebrity_MIAE_predicted_audio'+str(k)+'_'+str(epoch)+'.wav', restored_aud[0,0], fs)

    pickle.dump(test_loss_log,open('logs/audio/celebrity_MIAE_test_loss_log','wb'))
    pickle.dump(train_loss_log,open('logs/audio/celebrity_MIAE_train_loss_log','wb'))